...
import type { ServicePage as ServicePageConfig } from "@/lib/service-pages";
      ...
      <section className="border-b border-border bg-secondary">
        <div className="relative mx-auto max-w-7xl px-6 py-16 md:py-24">
          ...
          <div className="mt-14 max-w-4xl">
            <p className="text-sm font-semibold uppercase tracking-[0.25em] text-accent">{page.eyebrow}</p>
            <h1 className="mt-4 font-display text-4xl font-bold leading-tight tracking-tight md:text-6xl">{page.title}</h1>
            {page.price && <p className="mt-7 inline-flex rounded-full border border-accent/30 bg-accent/10 px-4 py-2 text-sm font-bold text-accent">{page.price}</p>}
          </div>
        </div>
      </section>
